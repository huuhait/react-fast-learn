...
import { Format, TableColumn } from '../types'
...
function Table<T>(props: Props<T>) {
  ...
  function routerLink(item: T) {
    ...
    let linkBuilder = props.linkBuilder
    let start_index = 0
                          ...
                          if (column.sort) {
                            return (
                              <div className="table-head-sort-caret">
                                <Icon type="caret-up" className={classNames({up: true, active: sortKey == column.key && sortType == 'up' })} />
                                <Icon type="caret-down" className={classNames({down: true, active: sortKey == column.key && sortType == 'down'})} />
                              </div>
                            )
                          }
                        })()}
      ...
      <div className="table-content">
        {(() => {
          if (props.dataSource.length === 0) {
            return <div className="table-empty w-full h-full flex flex-col items-center justify-center">
              <Icon type="print" />
              Empty
            </div>
          } else {
