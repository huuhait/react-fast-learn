    ...
    <LayoutContent className="page-product">
      <Container className="flex">
        ...
        <div className="w-6/12 pt-16">
          ...
          <span className="line-through text-2xl text-amber-700 opacity-40 mr-4">{Number(product.price).toLocaleString()}đ</span>
          <span className="font-bold text-2xl text-amber-700">{(Number(product.price) - (Number(product.price) * product.discount)).toLocaleString()}đ</span>
          <p className="pt-8">
            {product.description}
          </p>
