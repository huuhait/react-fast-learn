  ...
  const scopedSlotsRenderFunc = (item: Cart, column: TableColumn) => {
    switch (column.key) {
        ...
        return (
          <>
            <span className="line-through text-gray-500 mr-2">
              {Number(getProduct(item.product_id)?.price).toLocaleString()}
              đ
            </span>
            {Number(Number(getProduct(item.product_id)?.price) - Number(getProduct(item.product_id)?.price) * Number(getProduct(item.product_id)?.discount)).toLocaleString()}
            đ
          </>
        );
      case 'soluong':
      ...
      case 'tong':
        return (
          <>
            {Number(Number(Number(getProduct(item.product_id)?.price) - Number(getProduct(item.product_id)?.price) * Number(getProduct(item.product_id)?.discount)) * item.quantity).toLocaleString()}
            đ
          </>
        );
    }
    ...
    <LayoutContent className="page-gio-hang">
      <Container>
        <div className="flex my-10">
          <Table className="cart-table" dataSource={carts} columns={columns} scopedSlotsRenderFunc={(item, column) => scopedSlotsRenderFunc(item, column)} />
          <div className="cart-info ml-8">
            ...
            <Button className="block w-full rounded px-4 py-2 mt-8 text-center">
              {' '}
              {/* Dat hang here */}
              ĐẶT HÀNG
            </Button>
